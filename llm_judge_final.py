...
import pandas as pd
import csv
from inference_final import assign_grade_level, outputs, response, it_response, updated_prompts

# Import CSV of prompts and response
outputs = pd.read_csv("outputs.csv")
prompts = outputs.prompt.to_list()
it_response = outputs.it_response.to_list()
...
# login` (or set the `HF_TOKEN` env var) before loading it. # DONE
...
# TODO: wire up the LLM-as-judge pipeline for any open-ended scoring
...
def judge_safety(user_prompt: str, response: str, guideline: str) -> float:
    ...
    chat = [
        ...
        {"role": "assistant", "content": response}, # Uses format from inference_boilerplate.py
    ]
    ...
    return probabilities[0].item()  # P("Yes", i.e. the guideline is violated) - from YES_ID, NO_ID
...
# Move to a for loop since we have multiple prompts
user_prompt = updated_prompts
candidate_responses = [
    it_response
]

# Remove for loop; only one response per prompt
print(response)
scores = {
    category: judge_safety(user_prompt, response, guideline)
    for category, guideline in CUSTOM_GUIDELINES.items()
}
